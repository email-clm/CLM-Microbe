...
!-----------------------------------------------------------------------
    ...
    if (use_exit_spinup) then
       if (flag == 'read') then
          m = 20._r8
          do c = begc, endc
             ccs%soil1c(c) = ccs%soil1c(c) * 1._r8 !m
             ccs%soil2c(c) = ccs%soil2c(c) * 1._r8 !m
             ccs%soil3c(c) = ccs%soil3c(c) * 5._r8 !m
             ccs%soil4c(c) = ccs%soil4c(c) * 70._r8 !m
             if (use_c13) then
                ! adding code for 13C, 12/25/05, PET 
                cc13s%soil1c(c) = cc13s%soil1c(c) * 1._r8 !m
                cc13s%soil2c(c) = cc13s%soil2c(c) * 1._r8 !m
                cc13s%soil3c(c) = cc13s%soil3c(c) * 5._r8 !m
                cc13s%soil4c(c) = cc13s%soil4c(c) * 70._r8 !m
             end if
             cns%soil1n(c) = cns%soil1n(c) * 1._r8 !m
             cns%soil2n(c) = cns%soil2n(c) * 1._r8 !m
             cns%soil3n(c) = cns%soil3n(c) * 5._r8 !m
             cns%soil4n(c) = cns%soil4n(c) * 70._r8 !m
          end do
       end if
    end if
