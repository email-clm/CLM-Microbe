...
!
    ...
    !HUM_HOL
    real(r8), pointer :: qflx_surf_input(:) ! surface runoff input to hollow (mm H2O /s)
    real(r8), pointer :: qflx_lat_aqu(:)    ! lateral transport in aquifer (mm H2O /s)
   ...
   !HUM_HOL
    ...
    if ( found ) then
       ...
       if ((ctype(indexc) .eq. icol_roof .or. ctype(indexc) .eq. icol_road_imperv .or. &
          ...
          ! HUM_HOL
          write(iulog,*)'qflx_surf_input    = ',qflx_surf_input(:)
          write(iulog,*)'qflx_lat_aqu    = ',qflx_lat_aqu(:)
          !
          write(iulog,*)'qflx_qrgwl   = ',qflx_qrgwl(indexc)
          write(iulog,*)'qflx_drain   = ',qflx_drain(indexc)
