...
! ! PRIVATE TYPES
  ...
  integer :: ntimes
  logical :: do_harvest
  logical :: do_harvest_all  !added by JFM for harvest at first time step
...
!  modified by JFM for harvest at the first time step, 9/01/11
   use clm_time_manager, only : get_days_per_year, get_curr_date, get_step_size, get_nstep
!
! !ARGUMENTS:
   implicit none
...
! local pointers to implicit in arrays
   ...
   real(r8), pointer :: npool(:)              ! (gN/m2) N storage pool
...
! local pointers to implicit out arrays
   ...
   real(r8), pointer :: hrv_npool_to_litter(:)
!   real(r8), pointer :: hrv_cpool_to_litter(:)
...
!  modified by JFM for harvest at the first time step, 09/01/11
   integer :: year                      ! year for nstep+1
   integer :: mon                       ! month for nstep+1
   integer :: day                       ! day of month for nstep+1
   integer :: sec                       ! seconds into current date 
   integer :: nstep                     ! time step number
   integer :: p                         ! pft index
...
!-----------------------------------------------------------------------
   ...
   hrv_npool_to_litter              => pnf%hrv_npool_to_litter
   ...
   !-----------This block modified by JFM, 09/01/11
