...
#ifdef MICROBE
         sminn_vr(c,j) = sminn_vr(c,j) + cdons_min(c,j)
#endif
#else
...
#ifdef NPOOLMOD
      leafn(p) = leafn(p) - leafn_to_npool(p)*dt
      npool(p) = npool(p) + leafn_to_npool(p)*dt
#else
...
#ifdef NPOOLMOD
          livestemn(p)  = livestemn(p)  - livestemn_to_npool(p)*dt
          npool(p)      = npool(p)      + livestemn_to_npool(p)*dt
          livecrootn(p) = livecrootn(p) - livecrootn_to_npool(p)*dt
          npool(p)      = npool(p)      + livecrootn_to_npool(p)*dt
#else
...
#ifdef NPOOLMOD
          frootn(p)     = frootn(p)     - frootn_to_npool(p)*dt
	  npool(p)      = npool(p)      + frootn_to_npool(p)*dt
          livestemn(p)  = livestemn(p)  - livestemn_to_npool(p)*dt
          npool(p)      = npool(p)      + livestemn_to_npool(p)*dt
#else
