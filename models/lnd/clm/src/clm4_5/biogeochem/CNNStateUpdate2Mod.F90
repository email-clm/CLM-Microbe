...
!
#if(defined MICROBE)
   real(r8), pointer :: litter_n_to_don(:,:)         ! N fluxes from froot to don (gN/m3/s)
   real(r8), pointer :: m_frootn_to_don(:)         ! N fluxes from froot to don (gN/m3/s)
#endif
...
#if(defined MICROBE)
!write(*,*) "here 1"
         decomp_npools_vr(c,j,i_dom) = decomp_npools_vr(c,j,i_dom) + litter_n_to_don(c,j) * dt
!write(*,*) "here 2"
#endif         
...
#if(defined MICROBE)
!write(*,*) "froot ", frootn(p), p, m_frootn_to_don(p)
!       frootn(p)         = frootn(p)        - m_frootn_to_don(p) * dt
#endif
