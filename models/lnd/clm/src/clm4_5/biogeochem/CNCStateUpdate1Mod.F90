...
#ifdef CPOOLMOD
       cpool(p) = cpool(p) + excess_cflux_to_cpool(p)*dt
       cpool(p) = max(cpool(p) - cpool_to_turnover(p)*dt, 0._r8)
#endif
    end do
...
! !USES:
   ...
   use pftvarcon , only: npcropmin, nc3crop, exudate_frac
...
!
   ...
   real(r8), pointer :: cpool_turnover_to_litr_met_c(:,:)
...
!    cpool_turnover_to_litr_met_c      => ccisof%cpool_turnover_to_litr_met_c
...
#ifdef CPOOLMOD
    !fraction of turnover from cpool goes into labile soil carbon along froot profile
    cpool_turnover_to_litr_met_c(:,:)=0_r8
    do j = 1,nlevdecomp
        !column loop
        do fc = 1, num_soilc
          c = filter_soilc(fc)
          do fp=1,num_soilp
            p = filter_soilp(fp)
            cpool_turnover_to_litr_met_c(c,j) = cpool_turnover_to_litr_met_c(c,j) &
              + exudate_frac(ivt(p)) * cpool_to_turnover(p) * froot_prof(p,j)
          end do
          decomp_cpools_sourcesink(c,j,i_met_lit) = decomp_cpools_sourcesink(c,j,i_met_lit) + &
            cpool_turnover_to_litr_met_c(c,j) * dt
        end do
    end do
#endif
