...
!~ !write(iulog,*) "LBGC debug 8", lxdomunsat, cdocs_unsat(1,j), cdocs_unsat(2,j)
	...
	canaerch4bios(c,j) 		= canaerch4bios_unsat(c,j) + canaerch4bios_sat(c,j) 
	ccon_o2s(c,j) 			= ccon_o2s_unsat(c,j) + ccon_o2s_sat(c,j)
	ccon_ch4s(c,j) 			= ccon_ch4s_unsat(c,j) + ccon_ch4s_sat(c,j)
	ccon_h2s(c,j) 			= ccon_h2s_unsat(c,j) + ccon_h2s_sat(c,j)
	ccon_co2s(c,j)			= ccon_co2s_unsat(c,j) + ccon_co2s_sat(c,j)

	ch4_prod_ace_depth(c,j)	= ch4_prod_ace_depth_unsat(c,j) + ch4_prod_ace_depth_sat(c,j)
	ch4_prod_co2_depth(c,j)	= ch4_prod_co2_depth_unsat(c,j) + ch4_prod_co2_depth_sat(c,j)
	ch4_oxid_o2_depth(c,j)	= ch4_oxid_o2_depth_unsat(c,j) + ch4_oxid_o2_depth_sat(c,j)
	ch4_oxid_aom_depth(c,j)	= ch4_oxid_aom_depth_unsat(c,j) + ch4_oxid_aom_depth_sat(c,j)
	ch4_aere_depth(c,j)		= ch4_aere_depth_unsat(c,j) + ch4_aere_depth_sat(c,j)
	ch4_dif_depth(c,j)		= ch4_dif_depth_unsat(c,j) + ch4_dif_depth_sat(c,j)
	ch4_ebul_depth(c,j)		= ch4_ebul_depth_unsat(c,j) + ch4_ebul_depth_sat(c,j)
	
	co2_prod_ace_depth(c,j)	= co2_prod_ace_depth_unsat(c,j) + co2_prod_ace_depth_sat(c,j)
	co2_decomp_depth(c,j)	= co2_decomp_depth_unsat(c,j) + co2_decomp_depth_sat(c,j) 
	co2_cons_depth(c,j)		= co2_cons_depth_unsat(c,j) + co2_cons_depth_sat(c,j)
	co2_ebul_depth(c,j)		= co2_ebul_depth_unsat(c,j) + co2_ebul_depth_sat(c,j)
	co2_aere_depth(c,j)		= co2_aere_depth_unsat(c,j) + co2_aere_depth_sat(c,j)
	co2_dif_depth(c,j)		= co2_dif_depth_unsat(c,j) + co2_dif_depth_sat(c,j)
	
	o2_cons_depth(c,j)		= o2_cons_depth_unsat(c,j) + o2_cons_depth_sat(c,j)
	o2_aere_depth(c,j)		= o2_aere_depth_unsat(c,j) + o2_aere_depth_sat(c,j)
	o2_aere_oxid_depth(c,j)	= o2_aere_oxid_depth_unsat(c,j) + o2_aere_oxid_depth_sat(c,j)
	o2_decomp_depth(c,j)	= o2_decomp_depth_unsat(c,j) + o2_decomp_depth_sat(c,j)
	o2_dif_depth(c,j)		= o2_dif_depth_unsat(c,j) + o2_dif_depth_sat(c,j)
	
	h2_prod_depth(c,j)		= h2_prod_depth_unsat(c,j) + h2_prod_depth_sat(c,j)
	h2_cons_depth(c,j)		= h2_cons_depth_unsat(c,j) + h2_cons_depth_sat(c,j)
	h2_aere_depth(c,j)		= h2_aere_depth_unsat(c,j) + h2_aere_depth_sat(c,j)
	h2_diff_depth(c,j)		= h2_diff_depth_unsat(c,j) + h2_diff_depth_sat(c,j) 
	h2_ebul_depth(c,j)		= h2_ebul_depth_unsat(c,j) + h2_ebul_depth_sat(c,j)

	cdons_min(c,j)			= (cdocs_pre(c,j) - cdocs(c,j)) / cn_dom
	
      end do
      
      ch4_surf_aere(c)			= ch4_surf_aere_unsat(c) + ch4_surf_aere_sat(c)
      ch4_surf_ebul(c)			= ch4_surf_ebul_unsat(c) + ch4_surf_ebul_sat(c)
      ch4_surf_dif(c)			= ch4_surf_dif_unsat(c) + ch4_surf_dif_sat(c)
      ch4_surf_netflux(c)		= ch4_surf_netflux_unsat(c) + ch4_surf_netflux_sat(c)
      co2_surf_aere(c)			= co2_surf_aere_unsat(c) + co2_surf_aere_sat(c)
      co2_surf_ebul(c)			= co2_surf_ebul_unsat(c) + co2_surf_ebul_sat(c)
      co2_surf_dif(c)			= co2_surf_dif_unsat(c) + co2_surf_dif_sat(c)
      co2_surf_netflux(c)		= co2_surf_netflux_unsat(c) + co2_surf_netflux_sat(c)
      o2_surf_aere(c)			= o2_surf_aere_unsat(c) + o2_surf_aere_sat(c)
      o2_surf_dif(c)			= o2_surf_dif_unsat(c) + o2_surf_dif_sat(c)
      o2_surf_netflux(c)		= o2_surf_netflux_unsat(c) + o2_surf_netflux_sat(c)
      h2_surf_aere(c)			= h2_surf_aere_unsat(c) + h2_surf_aere_sat(c)
      h2_surf_ebul(c)			= h2_surf_ebul_unsat(c) + h2_surf_ebul_sat(c)
      h2_surf_dif(c)			= h2_surf_dif_unsat(c) + h2_surf_dif_sat(c)
      h2_surf_netflux(c)		= h2_surf_netflux_unsat(c) + h2_surf_netflux_sat(c)
      end do
