...
write(iulog,*) "xiaofeng here1: ", c,j, cdocs(c,j), cdons(c,j), decomp_cpools_vr(c,j,i_dom),decomp_npools_vr(c,j,i_dom)	    
      cdocs(c,j) = decomp_cpools_vr(c,j,i_dom) !* 1000.  ! gC/m3
!write(iulog, *) "xiaofeng here2 ", cdocs(c,j), decomp_cpools_vr(c,j,i_dom)     
      ...
      cdons(c,j) = decomp_npools_vr(c,j,i_dom) !* 1000.
...
!	separate carbon respiration to ten soil layers in saturated and unsaturated fraction
	do fc = 1,num_soilc
		...
		if(j > jwaterhead_unsat(c)) then
		finundated(c) = 0.95
		co2_decomp_depth_sat(c,j) = co2_decomp_depth_sat(c,j)  + (roothr_vr(c,j) + hr_vr(c,j)) * finundated(c)
		co2_decomp_depth_unsat(c,j) = co2_decomp_depth_unsat(c,j) + (roothr_vr(c,j) + hr_vr(c,j)) * (1. - finundated(c))
		
		o2_decomp_depth_unsat(c,j) = (roothr_vr(c,j) + hr_vr(c,j)) * (1. - finundated(c))
		o2_decomp_depth_sat(c,j) = (roothr_vr(c,j) + hr_vr(c,j)) * finundated(c)
		else
		finundated(c) = fsat(c)
		co2_decomp_depth_sat(c,j) = co2_decomp_depth_sat(c,j) + (roothr_vr(c,j) + hr_vr(c,j)) * finundated(c)
		co2_decomp_depth_unsat(c,j) = co2_decomp_depth_unsat(c,j) + (roothr_vr(c,j) + hr_vr(c,j)) * (1. - finundated(c))
...
! Xiaofeng replaced the following conditional code with control by oxygen with the control with soil moiture
	...
	AceProd = 2.0 / 3.0 * m_dAceProdACmax * ACConcentration / (ACConcentration + m_dKAce) &
		* (m_dACMinQ10 ** ((soiltemp(c,j) - 286.65) / 10.)) * pHeffect &
		* (1. - (caces_unsat(c,j) / (caces_unsat(c,j) + 100.))) * (log(minpsi/psi)/log(minpsi/maxpsi))
	ACH2Prod = AceProd / 6.0
	ACCO2Prod = 0.5 * AceProd
...
!write(iulog,*) "H2CH4Prod: ", H2CH4Prod
	...
	* cco2bios_unsat(c,j) / (cco2bios_unsat(c,j) + m_dKCO2ProdAce) &
	...
	if(ccon_h2s_unsat(c,j) > H2PlantFlux) then
	ccon_h2s_unsat(c,j) = ccon_h2s_unsat(c,j) - H2PlantFlux
	else
	H2PlantFlux = ccon_h2s_unsat(c,j)
	ccon_h2s_unsat(c,j) = ccon_h2s_unsat(c,j) - H2PlantFlux
	endif
...
!write(iulog,*) "m_drCH4Prod * (1 - m_dYAceMethanogens): ", m_drCH4Prod, " ", (1 - m_dYAceMethanogens) * AceCons, "H2CH4Prod: ", H2CH4Prod
	...
	* ccon_ch4s_unsat(c,j) / (ccon_ch4s_unsat(c,j) + m_dKCH4OxidCH4) &
	...
	if(ccon_ch4s_unsat(c,j) > CH4Oxid) then
	ccon_ch4s_unsat(c,j) = ccon_ch4s_unsat(c,j) - CH4Oxid
	else
	CH4Oxid = ccon_ch4s_unsat(c,j)
	ccon_ch4s_unsat(c,j) = ccon_ch4s_unsat(c,j) - CH4Oxid
	endif
...
!write(iulog,*) "ACCO2Prod: ", ACCO2Prod, "CH4Prod: ", m_drCH4Prod * (1 - m_dYAceMethanogens) * AceCons, "CH4Oxid: ", CH4Oxid
	...
	caces_unsat(c,j) = caces_unsat(c,j) + max(0., (1.0 - 2.0 * m_drCH4Prod * (1 - m_dYAceMethanogens)) * AceCons)
...
!write(iulog,*) "xiaofeng here ", c,j,cacebios_unsat(c,j),cco2bios_unsat(c,j),caerch4bios_unsat(c,j),canaerch4bios_unsat(c,j)
	...
	ch4_prod_ace_depth_unsat(c,j) 			= CH4Prod / get_step_size() 
	ch4_prod_co2_depth_unsat(c,j) 			= H2CH4Prod / get_step_size()
	ch4_oxid_o2_depth_unsat(c,j) 			= CH4Oxid / get_step_size()
	ch4_oxid_aom_depth_unsat(c,j) 			= AOMCH4Oxid / get_step_size()
	ch4_aere_depth_unsat(c,j) 				= CH4PlantFlux / get_step_size()
	ch4_dif_depth_unsat(c,j) 				= 0._r8 / get_step_size()
	ch4_ebul_depth_unsat(c,j) 				= CH4Ebull / get_step_size()

	co2_prod_ace_depth_unsat(c,j) 			= ACCO2Prod / get_step_size()
!	co2_decomp_depth_unsat(c,j) 		! this has been calclated in previous part of this subroutine
	co2_cons_depth_unsat(c,j) 				= 0._r8 / get_step_size()
	co2_ebul_depth_unsat(c,j) 				= 0._r8 / get_step_size()
	co2_aere_depth_unsat(c,j) 				= 0._r8 / get_step_size()
	co2_dif_depth_unsat(c,j) 				= 0._r8 / get_step_size()
 
	o2_cons_depth_unsat(c,j) 				= AerO2Cons + CH4O2Cons / get_step_size()
	o2_aere_depth_unsat(c,j) 				= O2PlantFlux / get_step_size()
	o2_aere_oxid_depth_unsat(c,j) 			= PlantO2Cons / get_step_size()
	o2_decomp_depth_unsat(c,j) 			= co2_decomp_depth_unsat(c,j) / get_step_size()
	o2_dif_depth_unsat(c,j) 				= 0._r8 / get_step_size()

	h2_prod_depth_unsat(c,j) 				= ACH2Prod / get_step_size()
	h2_cons_depth_unsat(c,j) 				= H2Cons / get_step_size()
	h2_aere_depth_unsat(c,j) 				= H2PlantFlux / get_step_size()
	h2_diff_depth_unsat(c,j) 				= H2PlantFlux / get_step_size()
	h2_ebul_depth_unsat(c,j) 				= H2PlantFlux / get_step_size()
else
...
!ch4, o2, co2, h2,
	...
	tem1 = 1. / tem1 * atmch4 / 16.
	tem2 = 1. / tem2 * atmo2 / 32.
	tem3 = 1. / tem3 * atmco2 / 44.
	tem4 = 1. / tem4 * atmh2 / 2.
...
if((soiltemp(c,jwaterhead_unsat(c)) < SHR_CONST_TKFRZ) .or. (jwaterhead_unsat(c) .lt. 9)) then
	ch4_surf_dif_unsat(c) 		= 0._r8 / get_step_size()
	ch4_surf_aere_unsat(c) 	= 0._r8 / get_step_size()
	ch4_surf_ebul_unsat(c) 	= 0._r8 / get_step_size()
	
	h2_surf_dif_unsat(c) 		= 0._r8 / get_step_size()
	h2_surf_aere_unsat(c) 	= 0._r8 / get_step_size()
	h2_surf_ebul_unsat(c) 		= 0._r8 / get_step_size()
	
	co2_surf_dif_unsat(c) 		= 0._r8 / get_step_size()
	co2_surf_aere_unsat(c)	= 0._r8 / get_step_size()
	co2_surf_ebul_unsat(c) 	= 0._r8 / get_step_size()
	
	o2_surf_dif_unsat(c) 		= 0._r8 / get_step_size()
	o2_surf_aere_unsat(c) 	= 0._r8 / get_step_size()	
else
	ch4_surf_dif_unsat(c) 	= ccon_ch4s_unsat(c,j) - tem1	! max(0._r8, (ccon_ch4s_unsat(c,1) - tem1))  ! solubility of ch4 is 0.0000227g/L
	ccon_ch4s_unsat(c,jwaterhead_unsat(c)) = ccon_ch4s_unsat(c,jwaterhead_unsat(c)) - ch4_surf_dif_unsat(c)
	ch4_surf_dif_unsat(c) 	= ch4_surf_dif_unsat(c) * dz(c,jwaterhead_unsat(c)) / get_step_size()
	...
	o2_surf_dif_unsat(c) 	= o2_surf_dif_unsat(c) * dz(c,jwaterhead_unsat(c)) / get_step_size()
	...
	co2_surf_dif_unsat(c) 	= co2_surf_dif_unsat(c) * dz(c,jwaterhead_unsat(c)) / get_step_size()

	h2_surf_dif_unsat(c) 	= ccon_h2s_unsat(c,jwaterhead_unsat(c)) - tem4 		! max(0._r8, (ccon_h2s_unsat(c,1) - tem2))  ! solubility of ch4 is 0.0000015/L
	ccon_h2s_unsat(c,jwaterhead_unsat(c)) = ccon_h2s_unsat(c,jwaterhead_unsat(c)) - h2_surf_dif_unsat(c)
	h2_surf_dif_unsat(c) 	= h2_surf_dif_unsat(c) * dz(c,jwaterhead_unsat(c)) / get_step_size()

	do j = 1,nlevsoi
	ch4_surf_aere_unsat(c)		= ch4_surf_aere_unsat(c) + ch4_aere_depth_unsat(c,j) * dz(c,j) / get_step_size()
	ch4_surf_ebul_unsat(c) 		= ch4_surf_ebul_unsat(c) + ch4_ebul_depth_unsat(c,j) * dz(c,j) / get_step_size()
	
	co2_surf_aere_unsat(c)		= co2_surf_aere_unsat(c) + co2_aere_depth_unsat(c,j) * dz(c,j) / get_step_size()
	co2_surf_ebul_unsat(c) 		= co2_surf_ebul_unsat(c) + co2_ebul_depth_unsat(c,j) * dz(c,j) / get_step_size()
	
	o2_surf_aere_unsat(c)			= o2_surf_aere_unsat(c) + o2_aere_depth_unsat(c,j) * dz(c,j) / get_step_size()
	
	h2_surf_aere_unsat(c)			= h2_surf_aere_unsat(c) + h2_aere_depth_unsat(c,j) * dz(c,j) / get_step_size()
	h2_surf_ebul_unsat(c) 			= h2_surf_ebul_unsat(c) + h2_ebul_depth_unsat(c,j) * dz(c,j) / get_step_size()
	end do
!end if
...
!write(iulog,*) "ccon_h2s(c,j): ",	ccon_h2s(c,j)	 
	...
	* cco2bios_sat(c,j) / (cco2bios_sat(c,j) + m_dKCO2ProdAce) &
	...
	if(cacebios_sat(c,j)<1e-10) then 
		cacebios_sat(c,j) = 1e-10
	end if
...
!	write(iulog,*) "m_drCH4Prod * (1 - m_dYAceMethanogens): ", m_drCH4Prod, " ", (1 - m_dYAceMethanogens) * AceCons, "H2CH4Prod: ", H2CH4Prod
	...
	* ccon_ch4s_sat(c,j) / (ccon_ch4s_sat(c,j) + m_dKCH4OxidCH4) &
	...
	ccon_ch4s_sat(c,j) = ccon_ch4s_sat(c,j) - CH4Ebull - CH4PlantFlux
	...
	caces_sat(c,j) = caces_sat(c,j) + max(0., (1.0 - 2.0 * m_drCH4Prod * (1 - m_dYAceMethanogens)) * AceCons)
...
!write(iulog,*) "grow, dead: ",	MethanotrophGrowth," ", MethanotrophDying," ", AOMMethanotrophGrowth," ", AOMMethanotrophDying
	...
	ch4_prod_ace_depth_sat(c,j) 			= CH4Prod / get_step_size()
	ch4_prod_co2_depth_sat(c,j) 			= H2CH4Prod / get_step_size()
	ch4_oxid_o2_depth_sat(c,j) 				= CH4Oxid / get_step_size()
	ch4_oxid_aom_depth_sat(c,j) 			= AOMCH4Oxid / get_step_size()
	ch4_aere_depth_sat(c,j) 				= CH4PlantFlux / get_step_size()
	ch4_dif_depth_sat(c,j) 				= 0._r8 / get_step_size()
	ch4_ebul_depth_sat(c,j) 				= CH4Ebull / get_step_size()

	co2_prod_ace_depth_sat(c,j) 			= ACCO2Prod / get_step_size()
!	co2_decomp_depth_sat(c,j) 			! calcuated in first part of this subroutine
	co2_cons_depth_sat(c,j) 				= 0._r8 / get_step_size()
	co2_ebul_depth_sat(c,j) 				= 0._r8 / get_step_size()
	co2_aere_depth_sat(c,j) 				= 0._r8 / get_step_size()
	co2_dif_depth_sat(c,j) 				= 0._r8 / get_step_size()

	o2_cons_depth_sat(c,j) 				= (AerO2Cons + CH4O2Cons) / get_step_size()
	o2_aere_depth_sat(c,j) 				= O2PlantFlux / get_step_size()
	o2_aere_oxid_depth_sat(c,j) 			= PlantO2Cons / get_step_size()
	o2_decomp_depth_sat(c,j) 				= co2_decomp_depth_sat (c,j) / get_step_size()		! mole / m3
	o2_dif_depth_sat(c,j) 					= 0._r8 / get_step_size()

	h2_prod_depth_sat(c,j) 				= ACH2Prod / get_step_size()
	h2_cons_depth_sat(c,j) 				= H2Cons / get_step_size()
	h2_aere_depth_sat(c,j) 				= H2PlantFlux / get_step_size()
	h2_diff_depth_sat(c,j) 					= H2PlantFlux / get_step_size()
	h2_ebul_depth_sat(c,j) 				= H2PlantFlux / get_step_size()
       end do
! average to grid level fluxes or state variables    
	tem1 = Henry_kHpc_w(1) * exp (Henry_C_w(1) * (1/soiltemp(c,1) - 1/kh_tbase))
	tem2 = Henry_kHpc_w(2) * exp (Henry_C_w(2) * (1/soiltemp(c,1) - 1/kh_tbase))
...
if(soiltemp(c,1) < SHR_CONST_TKFRZ) then
	ch4_surf_aere_sat(c)				= 0._r8 / get_step_size()
	ch4_surf_ebul_sat(c) 				= 0._r8 / get_step_size()
	ch4_surf_dif_sat(c) 				= 0._r8 / get_step_size()
		
	co2_surf_aere_sat(c)				= 0._r8 / get_step_size()
	co2_surf_ebul_sat(c) 				= 0._r8 / get_step_size()
	co2_surf_dif_sat(c) 				= 0._r8 / get_step_size()
		
	o2_surf_aere_sat(c)				= 0._r8 / get_step_size()
	o2_surf_dif_sat(c) 				= 0._r8 / get_step_size()
		
	h2_surf_aere_sat(c)				= 0._r8 / get_step_size()
	h2_surf_ebul_sat(c) 				= 0._r8 / get_step_size()
	h2_surf_dif_sat(c) 				= 0._r8 / get_step_size()
else
	ch4_surf_dif_sat(c) 	= ccon_ch4s_sat(c,1) - tem1 				!max(0._r8, (ccon_ch4s_sat(c,1) - tem1))  ! solubility of ch4 is 0.0000227g/L
	ccon_ch4s_sat(c,1) 	= ccon_ch4s_sat(c,1) - ch4_surf_dif_sat(c)
	ch4_surf_dif_sat(c) 	= ch4_surf_dif_sat(c) * dz(c,1) / get_step_size()
	...
	o2_surf_dif_sat(c) 	= o2_surf_dif_sat(c) * dz(c,1) / get_step_size()

	if(ccon_co2s_sat(c,1) > tem3) then
	...
	co2_surf_dif_sat(c) 	= co2_surf_dif_sat(c) * dz(c,1) / get_step_size()
	else
	co2_surf_dif_sat(c) 	= 0.0 * dz(c,1) / get_step_size()
	ccon_co2s_sat(c,1) 	= ccon_co2s_sat(c,1)	
	endif
	
	h2_surf_dif_sat(c) 	= ccon_h2s_sat(c,1) - tem4 				!max(0._r8, (ccon_h2s_sat(c,1) - tem2))  ! solubility of ch4 is 0.0000015/L
	ccon_h2s_sat(c,1) 	= ccon_h2s_sat(c,1) - h2_surf_dif_sat(c) 
	h2_surf_dif_sat(c) 	= h2_surf_dif_sat(c) * dz(c,1) / get_step_size()

	do j = 1,nlevsoi
	ch4_surf_aere_sat(c)	= ch4_surf_aere_sat(c) + ch4_aere_depth_sat(c,j) * dz(c,j) / get_step_size()
	ch4_surf_ebul_sat(c) 	= ch4_surf_ebul_sat(c) + ch4_ebul_depth_sat(c,j)  * dz(c,j) / get_step_size()
		
	co2_surf_aere_sat(c)	= co2_surf_aere_sat(c) + co2_aere_depth_sat(c,j) * dz(c,j) / get_step_size()
	co2_surf_ebul_sat(c) 	= co2_surf_ebul_sat(c) + co2_ebul_depth_sat(c,j) * dz(c,j) / get_step_size()
		
	o2_surf_aere_sat(c)	= o2_surf_aere_sat(c) + o2_aere_depth_sat(c,j) * dz(c,j) / get_step_size()
	 
	h2_surf_aere_sat(c)	= h2_surf_aere_sat(c) + h2_aere_depth_sat(c,j) * dz(c,j) / get_step_size()
	h2_surf_ebul_sat(c) 	= h2_surf_ebul_sat(c) + h2_ebul_depth_sat(c,j) * dz(c,j)  / get_step_size()
	end do
	...
	o2_surf_netflux_sat(c) 		= o2_surf_netflux_sat(c) + o2_surf_dif_sat(c) + o2_surf_aere_sat(c)
...
! keeping ACE consistent with DOC    
	    ...
	    !~ c = filter_soilc (fc)
	       ...
	       !~ sumace = 0.
	       !~ do j = 1,nlevdecomp
	       !~ sumdoc = sumdoc + cdocs_unsat(c,j) * dz(c,j)
	       !~ sumace = sumace + caces_unsat(c,j) * dz(c,j)
               !~ end do
	       !~ do j = 1,nlevdecomp
	       !~ if(sumdoc < 1e-20_r8) then
	       !~ caces_unsat(c,j) = 1e-21_r8
	       !~ else
	       !~ caces_unsat(c,j) =  cdocs_unsat(c,j) / sumdoc * sumace
	       !~ end if
               !~ end do	       
	       ...
	       !~ sumace = 0.
	       !~ do j = 1,nlevdecomp
	       !~ sumdoc = sumdoc + cdocs_sat(c,j) * dz(c,j)
	       !~ sumace = sumace + caces_sat(c,j) * dz(c,j)
               !~ end do
	       !~ do j = 1,nlevdecomp
	       !~ if(sumdoc < 1e-20_r8) then
	       !~ caces_sat(c,j) = 1e-21_r8
	       !~ else
	       !~ caces_sat(c,j) =  cdocs_sat(c,j) / sumdoc * sumace
	       !~ end if
               !~ end do	       
            !~ end do
...
!	write(iulog,*) "LBGC debug 5", cdocs_unsat(1,j), " ", qflx_lat_aqu(1), " ", cdocs_unsat(2,j), " ", qflx_lat_aqu(2)
	if(wa(1) > 1e-10 .and. abs(qflx_lat_aqu(1)) > 1e-10) then
		...
		lxaceunsat 			= qflx_lat_aqu(1) / sqrt(lat_flux_factor1) / vwc(1,j) * caces_unsat(1,j)! * get_step_size()
...
!write(iulog,*) "LBGC debug 8", lxdomunsat, cdocs_unsat(1,j), cdocs_unsat(2,j)
	...
	cdocs(c,j) 				= (cdocs_unsat(c,j) * (1. - finundated(c)) + cdocs_sat(c,j) * finundated(c)) 
	caces(c,j) 				= caces_unsat(c,j) * (1. - finundated(c)) + caces_sat(c,j) * finundated(c)
	caces_prod(c,j) 			= (caces_unsat_prod(c,j) * (1. - finundated(c)) + caces_sat_prod(c,j) * finundated(c)) !* get_step_size()
	cacebios(c,j) 			= cacebios_unsat(c,j) * (1. - finundated(c)) + cacebios_sat(c,j) * finundated(c)
	cco2bios(c,j) 			= cco2bios_unsat(c,j) * (1. - finundated(c)) + cco2bios_sat(c,j) * finundated(c)
	caerch4bios(c,j) 			= caerch4bios_unsat(c,j) * (1. - finundated(c)) + caerch4bios_sat(c,j) * finundated(c)
	canaerch4bios(c,j) 		= canaerch4bios_unsat(c,j) * (1. - finundated(c)) + canaerch4bios_sat(c,j) * finundated(c)
	ccon_o2s(c,j) 			= ccon_o2s_unsat(c,j) * (1. - finundated(c)) + ccon_o2s_sat(c,j) * finundated(c)
	ccon_ch4s(c,j) 			= ccon_ch4s_unsat(c,j) * (1. - finundated(c)) + ccon_ch4s_sat(c,j) * finundated(c)
	ccon_h2s(c,j) 			= ccon_h2s_unsat(c,j) * (1. - finundated(c)) + ccon_h2s_sat(c,j) * finundated(c)
	ccon_co2s(c,j)			= ccon_co2s_unsat(c,j) * (1. - finundated(c)) + ccon_co2s_sat(c,j) * finundated(c)

	ch4_prod_ace_depth(c,j)	= ch4_prod_ace_depth_unsat(c,j) * (1. - finundated(c)) + ch4_prod_ace_depth_sat(c,j) * finundated(c)
	ch4_prod_co2_depth(c,j)	= ch4_prod_co2_depth_unsat(c,j) * (1. - finundated(c)) + ch4_prod_co2_depth_sat(c,j) * finundated(c)
	ch4_oxid_o2_depth(c,j)	= ch4_oxid_o2_depth_unsat(c,j) * (1. - finundated(c)) + ch4_oxid_o2_depth_sat(c,j) * finundated(c)
	ch4_oxid_aom_depth(c,j)	= ch4_oxid_aom_depth_unsat(c,j) * (1. - finundated(c)) + ch4_oxid_aom_depth_sat(c,j) * finundated(c)
	ch4_aere_depth(c,j)		= ch4_aere_depth_unsat(c,j) * (1. - finundated(c)) + ch4_aere_depth_sat(c,j) * finundated(c)
	ch4_dif_depth(c,j)		= ch4_dif_depth_unsat(c,j) * (1. - finundated(c)) + ch4_dif_depth_sat(c,j) * finundated(c)
	ch4_ebul_depth(c,j)		= ch4_ebul_depth_unsat(c,j) * (1. - finundated(c)) + ch4_ebul_depth_sat(c,j) * finundated(c)
	
	co2_prod_ace_depth(c,j)	= co2_prod_ace_depth_unsat(c,j) * (1. - finundated(c)) + co2_prod_ace_depth_sat(c,j) * finundated(c)
	co2_decomp_depth(c,j)	= co2_decomp_depth_unsat(c,j) * (1. - finundated(c)) + co2_decomp_depth_sat(c,j) * finundated(c)
	co2_cons_depth(c,j)		= co2_cons_depth_unsat(c,j) * (1. - finundated(c)) + co2_cons_depth_sat(c,j) * finundated(c)
	co2_ebul_depth(c,j)		= co2_ebul_depth_unsat(c,j) * (1. - finundated(c)) + co2_ebul_depth_sat(c,j) * finundated(c)
	co2_aere_depth(c,j)		= co2_aere_depth_unsat(c,j) * (1. - finundated(c)) + co2_aere_depth_sat(c,j) * finundated(c)
	co2_dif_depth(c,j)		= co2_dif_depth_unsat(c,j) * (1. - finundated(c)) + co2_dif_depth_sat(c,j) * finundated(c)
	
	o2_cons_depth(c,j)		= o2_cons_depth_unsat(c,j) * (1. - finundated(c)) + o2_cons_depth_sat(c,j) * finundated(c)
	o2_aere_depth(c,j)		= o2_aere_depth_unsat(c,j) * (1. - finundated(c)) + o2_aere_depth_sat(c,j) * finundated(c)
	o2_aere_oxid_depth(c,j)	= o2_aere_oxid_depth_unsat(c,j) * (1. - finundated(c)) + o2_aere_oxid_depth_sat(c,j) * finundated(c)
	o2_decomp_depth(c,j)		= o2_decomp_depth_unsat(c,j) * (1. - finundated(c)) + o2_decomp_depth_sat(c,j) * finundated(c)
	o2_dif_depth(c,j)			= o2_dif_depth_unsat(c,j) * (1. - finundated(c)) + o2_dif_depth_sat(c,j) * finundated(c)
	
	h2_prod_depth(c,j)		= h2_prod_depth_unsat(c,j) * (1. - finundated(c)) + h2_prod_depth_sat(c,j) * finundated(c)
	h2_cons_depth(c,j)		= h2_cons_depth_unsat(c,j) * (1. - finundated(c)) + h2_cons_depth_sat(c,j) * finundated(c)
	h2_aere_depth(c,j)		= h2_aere_depth_unsat(c,j) * (1. - finundated(c)) + h2_aere_depth_sat(c,j) * finundated(c)
	h2_diff_depth(c,j)		= h2_diff_depth_unsat(c,j) * (1. - finundated(c)) + h2_diff_depth_sat(c,j) * finundated(c)
	h2_ebul_depth(c,j)		= h2_ebul_depth_unsat(c,j) * (1. - finundated(c)) + h2_ebul_depth_sat(c,j) * finundated(c)
      end do
      
      ch4_surf_aere(c)			= ch4_surf_aere_unsat(c) * (1. - finundated(c)) + ch4_surf_aere_sat(c) * finundated(c)
      ch4_surf_ebul(c)			= ch4_surf_ebul_unsat(c) * (1. - finundated(c)) + ch4_surf_ebul_sat(c) * finundated(c)
      ch4_surf_dif(c)			= ch4_surf_dif_unsat(c) * (1. - finundated(c)) + ch4_surf_dif_sat(c) * finundated(c)
      ch4_surf_netflux(c)		= ch4_surf_netflux_unsat(c) * (1. - finundated(c)) + ch4_surf_netflux_sat(c) * finundated(c)
      co2_surf_aere(c)			= co2_surf_aere_unsat(c) * (1. - finundated(c)) + co2_surf_aere_sat(c) * finundated(c)
      co2_surf_ebul(c)			= co2_surf_ebul_unsat(c) * (1. - finundated(c)) + co2_surf_ebul_sat(c) * finundated(c)
      co2_surf_dif(c)			= co2_surf_dif_unsat(c) * (1. - finundated(c)) + co2_surf_dif_sat(c) * finundated(c)
      co2_surf_netflux(c)		= co2_surf_netflux_unsat(c) * (1. - finundated(c)) + co2_surf_netflux_sat(c) * finundated(c)
      o2_surf_aere(c)			= o2_surf_aere_unsat(c) * (1. - finundated(c)) + o2_surf_aere_sat(c) * finundated(c)
      o2_surf_dif(c)			= o2_surf_dif_unsat(c) * (1. - finundated(c)) + o2_surf_dif_sat(c) * finundated(c)
      o2_surf_netflux(c)			= o2_surf_netflux_unsat(c) * (1. - finundated(c)) + o2_surf_netflux_sat(c) * finundated(c)
      h2_surf_aere(c)			= h2_surf_aere_unsat(c) * (1. - finundated(c)) + h2_surf_aere_sat(c) * finundated(c)
      h2_surf_ebul(c)			= h2_surf_ebul_unsat(c) * (1. - finundated(c)) + h2_surf_ebul_sat(c) * finundated(c)
      h2_surf_dif(c)			= h2_surf_dif_unsat(c) * (1. - finundated(c)) + h2_surf_dif_sat(c) * finundated(c)
      h2_surf_netflux(c)			= h2_surf_netflux_unsat(c) * (1. - finundated(c)) + h2_surf_netflux_sat(c) * finundated(c)
      end do
...
! added by xiaofeng on July 21 
       do j = 1,nlevdecomp
            do fc = 1,num_soilc
               c = filter_soilc(fc)
		cdons_min(c,j) = max(0._r8, decomp_npools_vr(c,j,i_dom) - (cdocs(c,j)) / cn_dom) / get_step_size() ! time step at second
!write(iulog,*) "xiaofeng here3: ", c,j, cdocs(c,j), cdons(c,j), decomp_cpools_vr(c,j,i_dom),decomp_npools_vr(c,j,i_dom)	    
		decomp_cpools_vr(c,j,i_dom) = (cdocs(c,j)) !/ 1000.  		! gC/m3
!write(iulog, *) "xiaofeng here4 ", cdocs(c,j), decomp_cpools_vr(c,j,i_dom)		
		decomp_npools_vr(c,j,i_dom) = (cdocs(c,j)) / cn_dom !/ 1000.	!
		sminn_vr(c,j) = sminn_vr(c,j) + cdons_min(c,j) * get_step_size() ! The time step at half-hour
           end do
      end do
...
! keep ten digital to keep the profile partitioning more accurate
	...
	hu_soil_tk(3) = 0.0454700332424132
	hu_soil_tk(4) = 0.0749674109862086
	hu_soil_tk(5) = 0.123600365102281
	hu_soil_tk(6) = 0.0108704034931662
	hu_soil_tk(7) = 0.0175128179162552
	hu_soil_tk(8) = 0.0275789692596763
	hu_soil_tk(9) = 0.0454700332424132
	hu_soil_tk(10) = 0.0749674109862086
	hu_soil_tk(11) = 0.0273829161127125
	hu_soil_tk(12) = 0.0962174489895685
	hu_soil_tk(13) = 0.203782551010432
	hu_soil_tk(14) = 0.0359806264484321
	hu_soil_tk(15) = 0.3
	hu_soil_tk(16) = 0.253938405368693
	hu_soil_tk(17) = 0.3
	hu_soil_tk(18) = 0.613290031589064
	hu_soil_tk(19) = 0.3
	hu_soil_tk(20) = 1.20576070139928
	...
	ho_soil_tk(3) = 0.0454700332424132
	ho_soil_tk(4) = 0.0749674109862086
	ho_soil_tk(5) = 0.0273829161127128
	ho_soil_tk(6) = 0.0962174489895678
	ho_soil_tk(7) = 0.203782551010432
	ho_soil_tk(8) = 0.0359806264484325
	ho_soil_tk(9) = 0.3
	ho_soil_tk(10) = 0.253938405368682
	ho_soil_tk(11) = 0.3
	ho_soil_tk(12) = 0.613290031589057
	ho_soil_tk(13) = 0.3
	ho_soil_tk(14) = 1.50576070139928
...
!write(iulog,*) "herebefore 1", cdocs_sat(1,1), cdocs_sat(1,2), cdocs_sat(1,3), cdocs_sat(1,4), cdocs_sat(1,5), cdocs_sat(1,6), cdocs_sat(1,7), cdocs_sat(1,8), cdocs_sat(1,9), cdocs_sat(1,10)
!write(iulog,*) "herebefore 2", cdocs_sat(2,1), cdocs_sat(2,2), cdocs_sat(2,3), cdocs_sat(2,4), cdocs_sat(2,5), cdocs_sat(2,6), cdocs_sat(2,7), cdocs_sat(2,8), cdocs_sat(2,9), cdocs_sat(2,10)
!write(iulog,*) "herebefore 0", cdocs_sat(2,1)+cdocs_sat(2,2)+cdocs_sat(2,3)+cdocs_sat(2,4)+cdocs_sat(2,5)+cdocs_sat(2,6)+cdocs_sat(2,7)+cdocs_sat(2,8)+cdocs_sat(2,9)+cdocs_sat(2,10)
do j = 1, 6
...
!~ !else
	...
	if(lxdocsat > 0 .and. abs(lxdocsat) > cdocs_sat_spruce1(j+6)) then
	lxdocsat = cdocs_sat_spruce1(j+6)
	end if
	if(lxdocsat < 0 .and. abs(lxdocsat) > cdocs_sat_spruce2(j)) then
	lxdocsat = -cdocs_sat_spruce2(j)
	end if
	...
	if(lxdonsat > 0 .and. abs(lxdonsat) > cdons_sat_spruce1(j+6)) then
	lxdonsat = cdons_sat_spruce1(j+6)
	end if
	if(lxdonsat < 0 .and. abs(lxdonsat) > cdons_sat_spruce2(j)) then
	lxdonsat = -cdons_sat_spruce2(j)
	end if		
	...
	if(lxacesat > 0 .and. abs(lxacesat) > caces_sat_spruce1(j+6)) then
	lxacesat = caces_sat_spruce1(j+6)
	end if
	if(lxacesat < 0 .and. abs(lxacesat) > caces_sat_spruce2(j)) then
	lxacesat = -caces_sat_spruce2(j)
	end if		
	...
	if(lx_o2s_sat_temp > 0 .and. abs(lx_o2s_sat_temp) > ccon_o2s_sat_spruce1(j+6)) then
	lx_o2s_sat_temp = ccon_o2s_sat_spruce1(j+6)
	end if
	if(lx_o2s_sat_temp < 0 .and. abs(lx_o2s_sat_temp) > ccon_o2s_sat_spruce2(j)) then
	lx_o2s_sat_temp = -ccon_o2s_sat_spruce2(j)
	end if	
	...
	if(lx_h2s_sat_temp > 0 .and. abs(lx_h2s_sat_temp) > ccon_h2s_sat_spruce1(j+6)) then
	lx_h2s_sat_temp = ccon_h2s_sat_spruce1(j+6)
	end if
	if(lx_h2s_sat_temp < 0 .and. abs(lx_h2s_sat_temp) > ccon_h2s_sat_spruce2(j)) then
	lx_h2s_sat_temp = -ccon_h2s_sat_spruce2(j)
	end if	
	...
	if(lx_co2s_sat_temp > 0 .and. abs(lx_co2s_sat_temp) > ccon_co2s_sat_spruce1(j+6)) then
	lx_co2s_sat_temp = ccon_co2s_sat_spruce1(j+6)
	end if
	if(lx_co2s_sat_temp < 0 .and. abs(lx_co2s_sat_temp) > ccon_co2s_sat_spruce2(j)) then
	lx_co2s_sat_temp = -ccon_co2s_sat_spruce2(j)
	end if	
	...
	if(lx_ch4s_sat_temp > 0 .and. abs(lx_ch4s_sat_temp) > ccon_ch4s_sat_spruce1(j+6)) then
	lx_ch4s_sat_temp = ccon_ch4s_sat_spruce1(j+6)
	end if
	if(lx_ch4s_sat_temp < 0 .and. abs(lx_ch4s_sat_temp) > ccon_ch4s_sat_spruce2(j)) then
	lx_ch4s_sat_temp = -ccon_ch4s_sat_spruce2(j)
	end if
...
!write(iulog,*) "hereafter0 1", cdocs_sat_spruce1(1), cdocs_sat_spruce1(2), cdocs_sat_spruce1(3), cdocs_sat_spruce1(4), cdocs_sat_spruce1(5), cdocs_sat_spruce1(6), cdocs_sat_spruce1(7), cdocs_sat_spruce1(8), cdocs_sat_spruce1(9), cdocs_sat_spruce1(10), cdocs_sat_spruce1(11), cdocs_sat_spruce1(12), cdocs_sat_spruce1(13), cdocs_sat_spruce1(14), cdocs_sat_spruce1(15), cdocs_sat_spruce1(16), cdocs_sat_spruce1(17), cdocs_sat_spruce1(18), cdocs_sat_spruce1(19), cdocs_sat_spruce1(20)
!write(iulog,*) "hereafter0 2", cdocs_sat_spruce2(1), cdocs_sat_spruce2(2), cdocs_sat_spruce2(3), cdocs_sat_spruce2(4), cdocs_sat_spruce2(5), cdocs_sat_spruce2(6), cdocs_sat_spruce2(7), cdocs_sat_spruce2(8), cdocs_sat_spruce2(9), cdocs_sat_spruce2(10), cdocs_sat_spruce2(11), cdocs_sat_spruce2(12), cdocs_sat_spruce2(13), cdocs_sat_spruce2(14)
...
!~ ! aggregate the 20/14 layers bgc variables to 10 layers
	...
	!~ ccon_ch4s_sat(2,10) = ccon_ch4s_sat_spruce2(14)	
!write(iulog,*) "hereafter 1", cdocs_sat(1,1), cdocs_sat(1,2), cdocs_sat(1,3), cdocs_sat(1,4), cdocs_sat(1,5), cdocs_sat(1,6), cdocs_sat(1,7), cdocs_sat(1,8), cdocs_sat(1,9), cdocs_sat(1,10)
!write(iulog,*) "hereafter 2", cdocs_sat(2,1), cdocs_sat(2,2), cdocs_sat(2,3), cdocs_sat(2,4), cdocs_sat(2,5), cdocs_sat(2,6), cdocs_sat(2,7), cdocs_sat(2,8), cdocs_sat(2,9), cdocs_sat(2,10)
