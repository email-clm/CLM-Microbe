...
! Initialize evaluation index with default values.
    ...
    ! Initialize the model for the main optimization.
        x_lower = (/ 78d0,33d0,27d0,27d0,27d0 /)
        x_upper = (/ 102d0,45d0,45d0,45d0,45d0 /)
...
! constraints g<=0
    g(1) = 85.334407d0 + 0.0056858d0*x(2)*x(5) + 0.0006262d0*x(1)*x(4) &
         - 0.0022053d0*x(3)*x(5) - 92d0
    g(2) = -85.334407d0 - 0.0056858d0*x(2)*x(5) - 0.0006262d0*x(1)*x(4) &
         + 0.0022053d0*x(3)*x(5)
    g(3) = 80.51249d0 + 0.0071317d0*x(2)*x(5) + 0.0029955d0*x(1)*x(2) &
         + 0.0021813d0*x(3)**2 - 110d0
    g(4) = -80.51249d0 - 0.0071317d0*x(2)*x(5) - 0.0029955d0*x(1)*x(2) &
         - 0.0021813d0*x(3)**2 + 90d0
    g(5) = 9.300961d0 + 0.0047026d0*x(3)*x(5) + 0.0012547d0*x(1)*x(3) &
         + 0.0019085d0*x(3)*x(4) - 25d0
    g(6) = -9.300961d0 - 0.0047026d0*x(3)*x(5) - 0.0012547d0*x(1)*x(3) &
         - 0.0019085d0*x(3)*x(4) + 20d0
...
!  ========================================================================
    ...
    write(FID,"(a,f10.3)") '# g04:  f(x) = ', fout
    write(FID,"(a)") '# i    x(i)'

    do i = 1,size(Solution)
        write(FID, "(i3,f17.12)") i, Solution(i)
    end do
