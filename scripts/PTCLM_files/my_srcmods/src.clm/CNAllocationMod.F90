...
!5/17/2011, DMR
!update to merge PET's changes for NACP
...
! !USES:
   ...
   use nanMod          , only: nan
   use pftvarcon       , only: bdnr_d, dayscrecover, rc_npool
! !ARGUMENTS:
   implicit none
...
!
   ...
   real(r8), pointer :: npool(:)      ! (kgN/m2) plant pool of allocatable N
                                      !    added by DMR 5/17/2011 
                                      !   (to merge PET change 2/1/2011)
...
! !OTHER LOCAL VARIABLES:
   ...
   real(r8):: r, rc                !resistance terms for plant nuptake
                                   !DMR 5/17/2011 to add PET changes from 2/1/11
...
!-----------------------------------------------------------------------
   ...
   do fp=1,num_soilp
      ...
      !5/17/2011 - DMR
      !add PET's changes from 2/1/2011
      !modify retrans treatment to always put all retransn into npool
      !also, do not reduce plant_ndemand by retransn_to_npool
      !original code is commented below the new code

      retransn_to_npool(p)=retransn(p)/dt
   ...
   do fp=1,num_soilp
      ...
      !DMR 5/17/2011 - add PET's changes from 2/1/2011 
      !make plant_nalloc a function of npool
      !rc = 10.0_r8            !DMR 7/5/11 - rc now a model parameter
      rc = rc_npool
      r  = max(1._r8,rc/max(npool(p),1e-9_r8))
      plant_nalloc(p) = plant_ndemand(p)/r
