...
#              str(ygrid)+','+str(ygrid+numypts-1)+' '+gridfile_orig+' '+gridfil#e_new)
##gridfile_new_nc = NetCDF.NetCDFFile(gridfile_new, "a")
#numlon = getvar(gridfile_new, 'NUMLON')
#longxy = getvar(gridfile_new, 'LONGXY')
#latixy = getvar(gridfile_new, 'LATIXY')
#latn   = getvar(gridfile_new, 'LATN')
#lats   = getvar(gridfile_new, 'LATS')
#lone   = getvar(gridfile_new, 'LONE')
#lonw   = getvar(gridfile_new, 'LONW')
#area   = getvar(gridfile_new, 'AREA')
...
#        latn[j][i] = lat+resy/2+j*resy
#        lats[j][i] = lat-resy/2+j*resy
#        lone[j][i] = lon+resx/2+i*resx
#        lonw[j][i] = lon-resx/2+i*resx
#        area[j][i] = (110.567+1.102*abs(lat)/90)*resy*111.321*math.cos((lon+i*resx)*math.pi/180)*resx

#ierr = putvar(gridfile_new, 'NUMLON', numxpts)
#ierr = putvar(gridfile_new, 'LONGXY', longxy)
#ierr = putvar(gridfile_new, 'LATIXY', latixy)
#ierr = putvar(gridfile_new, 'LATN', latn)
#ierr = putvar(gridfile_new, 'LATS', lats)
#ierr = putvar(gridfile_new, 'LONE', lone)
#ierr = putvar(gridfile_new, 'LONW', lonw)
#ierr = putvar(gridfile_new, 'AREA', area)
...
#numlon   = getvar(fracfile_new, 'NUMLON')
#landfrac = getvar(fracfile_new, 'LANDFRAC')
#landmask = getvar(fracfile_new, 'LANDMASK')
#longxy   = getvar(fracfile_new, 'LONGXY')
#latixy   = getvar(fracfile_new, 'LATIXY')
...
#ierr = putvar(fracfile_new, 'NUMLON', numxpts)
#ierr = putvar(fracfile_new, 'LANDFRAC', landfrac)
#ierr = putvar(fracfile_new, 'LANDMASK', landmask)
#ierr = putvar(fracfile_new, 'LONGXY', longxy)
#ierr = putvar(fracfile_new, 'LATIXY', latixy)
...
if (options.clm40):
    ...
    surffile_new  = options.ccsm_input+'/lnd/clm2/surfdata/surfdata_'+str(numxpts)+'x' \
    ...
    surffile_new = options.ccsm_input+'/lnd/clm2/surfdata_map/surfdata_'+str(numxpts)+ \
                   'x'+str(numypts)+'pt_'+options.mycase+'_simyr'+str(mysimyr)+'.nc'
...
pct_pft      = getvar(surffile_new, 'PCT_PFT') 
...
ierr = putvar(surffile_new, 'PCT_PFT', pct_pft)
...
if (options.compset == 'I20TRCLM45CN' or options.compset == 'I20TRCN'):
    ...
    if (options.clm40):
        ...
        pftdyn_new = options.ccsm_input+'/lnd/clm2/surfdata_map/' \
          +'surfdata.pftdyn_'+str(numxpts)+'x'+str(numypts)+'pt_'+options.mycase+'.nc'
    ...
    pct_pft      = getvar(pftdyn_new, 'PCT_PFT')
    ...
    pct_pft_1850     = getvar(surffile_new, 'PCT_PFT')
    ...
    ierr = putvar(pftdyn_new, 'PCT_PFT', pct_pft)
